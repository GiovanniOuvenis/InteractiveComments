...
const Comment = (props) => {
  ...
  useEffect(() => {
    if (props.typeOfComment === "reply") {
      ...
      alignRight.current.classList.add("alignRight");
    }
  });
  ...
  return (
    <>
      ...
      <div className="comment-container" ref={alignRight}>
      ...
      {replyForm && (
        <PostComment
          ...
          action={editOrReply}
          text={content}
          nameOfClass="postReply"
        ></PostComment>
      )}
      {replies.length > 0 && (
        <>
          {replies.map((reply) => {
            return <Comment typeOfComment="reply" comment={reply} />;
          })}
        </>
      )}
    </>
